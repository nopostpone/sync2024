...
void pushup(int pos) {
    ...
    for (int i : {0, 1}) {
        ...
        if ((i == 1 and s[lst].sum == s[lst].r - s[lst].l + 1) or (i == 0 and s[lst].sum == 0))
            s[pos].ls[i] += s[rst].ls[i];

        s[pos].rs[i] = s[rst].rs[i];
        if ((i == 1 and s[rst].sum == s[rst].r - s[rst].l + 1) or (i == 0 and s[rst].sum == 0))
...
void solve() {
    ...
    for (int i = 1; i <= m; i++) {
        ...
        if (opt >= 0 and opt <= 2) {
            ...
            res.push_back(querymx(l, r).ss[1]);
        }
    }
    for (auto i : res) {
        cout << i << endl;
