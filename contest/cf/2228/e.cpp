...
void solve() {
    ...
    a = std::vector(a.begin() + _l, a.begin() + _r);
    n = a.size();
    ...
    // std::cerr << comb.binom(need, k) << "\n";

    Z w = comb.binom(need + k - 1, k - 1);
    Z c1 = comb.binom(need + k - 1, k);
    Z c2 = comb.binom(need + k - 1, k + 1);
    ...
    for (int i = 0; i < n; i++) {
        ...
        ans += pre * pre * w + (2 * pre * cnt + cnt) * c1 + cnt * (cnt + 1) * c2;
    }
    std::cout << ans << "\n";
}
