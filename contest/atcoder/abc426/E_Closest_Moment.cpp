...
double get(P s1, P t1, P s2, P t2) {
    P a = s1 - s2;
    P b = t1 - t2;
    if (a == b) {
        return length(a);
    }
    return distancePS(P(), Line(a, b));
}
...
void solve() {
    ...
    double d1 = distance(s1, t1);
    double d2 = distance(s2, t2);
    ...
    double ans = min(get(s1, t1, s2, s2 + d1 * normalize(t2 - s2)),
                     get(t1, t1, s2 + d1 * normalize(t2 - s2), t2));
